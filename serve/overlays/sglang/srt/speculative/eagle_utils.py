...
def eagle_sample(
    ...
    if sampling_info.is_all_greedy or _is_cpu or _is_hip or _is_xpu:
        ...
        from sglang.srt.qwenfast import sparse_verify_topk

        sparse_km = (
            sparse_verify_topk(batch, sampling_info, verify_input, next_token_logits)
            if use_rejection_sampling
            else None
        )
        if sparse_km is not None:
            ...
            if not (
                coins.dtype == torch.float32
                and coins.dim() == 2
                and coins.shape[0] == bs
                and coins.shape[1] >= verify_input.draft_token_num - 1
                and coins.stride(1) == 1
                and coins_for_final_sampling.dtype == torch.float32
                and coins_for_final_sampling.is_contiguous()
                and coins_for_final_sampling.numel() == bs
            ):
                sparse_km = None
        if sparse_km is not None:
            # top-k/top-p renorm + chain rejection sampling on each row's top candidates
            # (same kept sets, probabilities and accept/resample rules as the dense path)
            from sglang.srt.qwenfast import ext as _qf_ext

            topk_vals, topk_idx = torch.topk(next_token_logits, sparse_km, dim=-1)
            _qf_ext().spec_topk_chain(
                topk_vals.float().contiguous(),
                topk_idx,
                sampling_info.temperatures.contiguous(),
                sampling_info.top_ks.contiguous(),
                sampling_info.top_ps.contiguous(),
                bool(sampling_info.need_top_p_sampling),
            ...
            del topk_vals, topk_idx, coins, coins_for_final_sampling
        else:
