...
class Qwen4ExpForConditionalGeneration(Qwen3VLForConditionalGeneration):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        # Startup-only index: the checkpoint has ~296K tensors. Looking up
        # the exact expert/projection avoids scanning every expert's mapping
        # for each tensor. The selected tuple and weight-loader call are
        # unchanged; unrecognized layouts retain the original fallback.
        expert_mapping_index = {entry[1]: entry for entry in expert_params_mapping}
        expert_mapping_pattern = re.compile(r"experts\.\d+\.(?:gate_proj|down_proj|up_proj)\.")
        ignore_suffixes = (
            ".bias",
            "_bias",
            ".k_scale",
        ...
        for name, loaded_weight in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                if not is_fused_expert:
                    expert_match = expert_mapping_pattern.search(name)
                    if expert_match is not None:
                        entry = expert_mapping_index.get(expert_match.group())
                        if entry is not None:
                            current_expert_params_mapping = (entry,)
