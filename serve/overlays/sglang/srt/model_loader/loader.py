...
class DefaultModelLoader(BaseModelLoader):
    ...
    def _get_all_weights(
        ...
        # Qwen4-Exp's MTP loader discards every non-MTP tensor. Select the
        # exact same keys before get_tensor(), rather than constructing and
        # discarding ~296K tensors from the full target checkpoint again.
        # Keep the original iterator for missing indexes/other architectures.
        if (
            model.__class__.__name__ == "Qwen4ExpForCausalLMMTP"
            and not getattr(model, "secondary_weights", ())
            and self.load_config.draft_model_idx is None
        ):
            index_path = os.path.join(model_config.model_path, "model.safetensors.index.json")
            if os.path.isfile(index_path):
                with open(index_path) as index_file:
                    weight_map = json.load(index_file)["weight_map"]
                selected = collections.defaultdict(list)
                for name, filename in weight_map.items():
                    if "mtp" in name and "rotary_emb.inv_freq" not in name:
                        selected[filename].append(name)
                if selected:
                    from safetensors import safe_open

                    logger.info("Qwen MTP indexed load: %d tensors from %d shards (unchanged checkpoint)",
                                sum(map(len, selected.values())), len(selected))
                    if self.counter_before_loading_weights == 0.0:
                        self.counter_before_loading_weights = time.perf_counter()
                    for filename in sorted(selected):
                        with safe_open(os.path.join(model_config.model_path, filename), framework="pt", device="cpu") as shard:
                            for name in sorted(selected[filename]):
    ...
    def load_model(
        ...
        *,
        ...
        if model.__class__.__name__ == "Qwen4ExpForCausalLMMTP":
            # The slow full-checkpoint scan happened to trigger Python cycle
            # collection. With indexed loading, collect unreachable temporary
            # model tensors explicitly BEFORE KV-cache capacity is calculated.
            # Live weights are unaffected; empty_cache only releases unused
            # allocator blocks. Otherwise ~6.6 GiB is miscounted as live until
            # graph capture, silently shrinking the KV token pool.
            before = torch.cuda.memory_allocated() if target_device.type == "cuda" else 0
            collected = gc.collect()
            if target_device.type == "cuda":
                torch.cuda.synchronize()
                torch.cuda.empty_cache()
            after = torch.cuda.memory_allocated() if target_device.type == "cuda" else 0
            logger.info("Qwen MTP startup cleanup: %d cycles collected, %.2f GiB temporary tensors released", collected, (before-after) / (1024**3))
