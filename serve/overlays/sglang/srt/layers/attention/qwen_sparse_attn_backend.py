...
def _resolve_flash_attn_varlen_func():
    ...
    # Preserve the workstation SM120 fallback dispatcher from the tested profile.
...
class QwenSparseAttnBackend(AttentionBackend):
    ...
    def _require_chain_speculation(self, forward_mode, spec_info) -> None:
        ...
        required = draft_tokens + self.compress_ratio - 1
        if required > ring_size:
            raise NotImplementedError(
                f"QSA verification needs {required} slots including the prior "
                f"partial compression group; allocated {ring_size}"
            )
